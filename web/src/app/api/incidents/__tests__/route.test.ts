...
describe('Incidents API', () => {
  beforeEach(() => {
    jest.clearAllMocks()
    ;(verifyAuth as jest.Mock).mockResolvedValue({ success: true, user: { id: 'admin-1', role: 'ADMIN' } })
  })
  ...
  describe('GET /api/incidents', () => {
    ...
    it('should return paginated incidents with filters', async () => {
      prismaMock.incident.findMany.mockResolvedValue([{ id: 'inc-1', status: 'REPORTED' }] as any)
      prismaMock.incident.count.mockResolvedValue(1)

      const req = new NextRequest('http://localhost:3000/api/incidents?status=REPORTED&page=2&limit=10')
      const response = await GET(req)
      ...
      expect(prismaMock.incident.findMany).toHaveBeenCalledWith(
        expect.objectContaining({ skip: 10, take: 10, where: { status: 'REPORTED' } })
      )
    })
  })
  ...
  describe('POST /api/incidents', () => {
    ...
    it('should create an incident successfully', async () => {
      ...
      prismaMock.incident.create.mockResolvedValue({ id: 'new-inc', ...validBody } as any)
