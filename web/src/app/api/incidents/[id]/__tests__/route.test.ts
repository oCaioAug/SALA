...
describe('Incidents [id] API', () => {
  beforeEach(() => {
    ...
    prismaMock.user.findUnique.mockResolvedValue({ id: 'admin-1', role: 'ADMIN', email: 'admin@example.com' } as any)
  })

  describe('GET /api/incidents/[id]', () => {
    it('should return 404 if incident does not exist', async () => {
      prismaMock.incident.findUnique.mockResolvedValue(null)
      const req = new NextRequest('http://localhost:3000/api/incidents/invalid-id')
    ...
    it('should return incident details', async () => {
      ...
      const response = await GET(req, { params: Promise.resolve({ id: 'inc-1' }) })
      const data = await response.json()
  ...
  describe('PUT /api/incidents/[id]', () => {
    it('should prevent non-authorized users from updating', async () => {
      ...
      const req = new NextRequest('http://localhost:3000/api/incidents/inc-1', { method: 'PUT', body: JSON.stringify({}) })
      const response = await PUT(req, { params: Promise.resolve({ id: 'inc-1' }) })
    ...
    it('should allow admin to update incident', async () => {
      ...
      prismaMock.incident.findUnique.mockResolvedValue(mockIncident as any)
      prismaMock.incident.update.mockResolvedValue({ ...mockIncident, status: 'RESOLVED' } as any)
  ...
  describe('DELETE /api/incidents/[id]', () => {
    it('should return 403 for non-admins', async () => {
      ...
      const req = new NextRequest('http://localhost:3000/api/incidents/inc-1', { method: 'DELETE' })
      const response = await DELETE(req, { params: Promise.resolve({ id: 'inc-1' }) })
    ...
    it('should delete incident if admin', async () => {
      ...
      expect(response.status).toBe(200)
      expect(prismaMock.incident.delete).toHaveBeenCalledWith({ where: { id: 'inc-1' } })
    })
  })
})
