...
describe('Items [id] API', () => {
  ...
  describe('GET /api/items/[id]', () => {
    ...
    it('should return item details', async () => {
      ...
      const response = await GET(req, { params: Promise.resolve({ id: 'item-1' }) })
      const data = await response.json()
  ...
  describe('PUT /api/items/[id]', () => {
    it('should return 404 if item does not exist', async () => {
      ...
      const response = await PUT(req, { params: Promise.resolve({ id: 'invalid-id' }) })
      const data = await response.json()
    ...
    it('should update item successfully', async () => {
      const mockItem = { id: 'item-1', name: 'Projector' }
      prismaMock.item.findUnique.mockResolvedValue(mockItem as any)
      prismaMock.item.update.mockResolvedValue({ ...mockItem, name: 'Projector 4K' } as any)
  ...
  describe('DELETE /api/items/[id]', () => {
    it('should delete item and its images', async () => {
      ...
      const req = new NextRequest('http://localhost:3000/api/items/item-1', { method: 'DELETE' })
