...
describe('Notifications API', () => {
  ...
  describe('GET /api/notifications', () => {
    ...
    it('should retrieve notifications by user email', async () => {
      prismaMock.user.findUnique.mockResolvedValue({ id: 'user-1', email: 'user@example.com' } as any)
      prismaMock.notification.findMany.mockResolvedValue([{ id: 'notif-1' }] as any)

      const req = new NextRequest('http://localhost:3000/api/notifications?userId=user@example.com&isRead=false&limit=10')
      const response = await GET(req)
    ...
    it('should retrieve notifications by user id', async () => {
      ...
      const req = new NextRequest('http://localhost:3000/api/notifications?userId=user-1')
      const response = await GET(req)
    ...
    it('should return 404 if user not found', async () => {
      ...
      const req = new NextRequest('http://localhost:3000/api/notifications?userId=inv')
      const response = await GET(req)
  ...
  describe('POST /api/notifications', () => {
    ...
    it('should create notification successfully', async () => {
      prismaMock.notification.create.mockResolvedValue({ id: 'notif-1', ...validBody } as any)
