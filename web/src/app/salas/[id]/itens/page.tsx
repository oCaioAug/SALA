...
const RoomItemsPage: React.FC = () => {
  ...
  const handleAddItem = async (itemData: Omit<Item, 'id' | 'createdAt' | 'updatedAt'>, imageData?: any) => {
    try {
  ...
  const handleUpdateItem = async (itemData: Omit<Item, 'id' | 'createdAt' | 'updatedAt'>, imageData?: any) => {
    ...
    >
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-slate-600 dark:text-gray-400">{items.length} itens cadastrados</p>
            </div>
          ...
          <Card>
              <CardContent className="p-12 text-center">
                <div className="text-slate-500 dark:text-gray-400 mb-4">
                  ...
                  <p className="text-xl mb-2 text-slate-900 dark:text-white">Nenhum item cadastrado</p>
                  <p className="text-sm text-slate-600 dark:text-gray-400">Comece adicionando itens para esta sala</p>
                </div>
                <Button onClick={() => setIsAddItemModalOpen(true)}>
                      ...
                      <div className="p-6">
                        <div className="flex items-start justify-between mb-4">
                          <div className="flex-1">
                            ...
                            <p className="text-sm text-slate-600 dark:text-gray-400">Quantidade: {item.quantity}</p>
                          </div>
                        </div>
                        ...
                        {item.description && (
                          <p className="text-slate-700 dark:text-gray-300 text-sm mb-4 mt-4">{item.description}</p>
                        )}

                        {item.specifications && item.specifications.length > 0 && (
                          <div className="mt-4">
                            <p className="text-xs text-slate-600 dark:text-gray-500 font-medium mb-2">Especificações:</p>
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="flex gap-3 pt-4">
        <Button type="submit" className="flex-1" disabled={uploading}>
          {uploading ? 'Salvando...' : item ? 'Atualizar Item' : 'Adicionar Item'}
        </Button>
