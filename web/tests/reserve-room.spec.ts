...
test.describe('Room Reservation Flow', () => {
  ...
  test.beforeAll(async () => {
    ...
    if (!user) {
      throw new Error('Test user user@sala.com not found. Did you run the seed?');
    }
  ...
  test('user can navigate to room and make a reservation', async ({ page }) => {
    ...
    const roomCard = page.locator('.group', { hasText: 'Laboratório de Robótica' }).first();
    ...
    const reservationsLink = page.locator('button', { hasText: /(Agendamentos|View Reservations)/i });
    await page.screenshot({ path: 'test-results/04-before-agendamentos-click.png' });
    ...
    const newReservationBtn = page.locator('button', { hasText: /Nova Reserva|New Reservation/i });
    await page.screenshot({ path: 'test-results/06-before-nova-reserva-click.png' });
    ...
    await expect(page.locator('text="E2E Test Reservation"')).toBeVisible({ timeout: 10000 });
